...
function renderDetail(){const s=activeSector,src=SRC[s.source];$('#detailSector').textContent=s.name;$('#detailScore').textContent=s.babel;$('#detailPriority').textContent=s.priority;$('#detailGrid').innerHTML=`
 <article class="panel"><span>DECISÃO</span><h4>${s.priority==='ATAQUE'?'Atacar com prova rápida e processo específico.':s.priority==='ESTRATÉGICO'?'Venda consultiva com prova técnica e ROI.':'Qualificar antes de investir ciclo comercial.'}</h4><p>Use uma única fricção operacional como porta de entrada. Não tente explicar a Babel inteira.</p></article>
 <article class="panel"><span>DECISOR</span><h4>${esc(s.decision)}</h4><p>Acesso modelado: ${s.access}/100. Quanto mais enterprise, mais importante mapear sponsor, usuário e área técnica.</p></article>
 <article class="panel"><span>DOR FUNDAMENTAL</span><h4>${esc(s.pain)}</h4><p>Pressão operacional ${s.ops}/100 · automação ${s.automation}/100.</p></article>
 <article class="panel"><span>PONTO DE OURO</span><h4>${esc(s.gold)}</h4><p>O valor aparece quando um sinal já existe, mas a ação certa ainda não acontece com contexto suficiente.</p></article>
 <article class="panel"><span>ARGUMENTO</span><h4>${esc(s.opening)}</h4><p>Use como hipótese e escute a resposta. A pergunta vale mais que uma afirmação genérica.</p></article>
 <article class="panel"><span>RISCO</span><h4>${esc(s.risk)}</h4><p>Dificuldade comercial ${s.difficulty}/100 · maturidade ${s.maturity}/100.</p></article>`;
