...
// Remove experiments anteriores e qualquer versão prévia do Call OS.
html=html
...
const fragment=fs.readFileSync('scripts/call-os-v2.fragment.html','utf8');
if(!fragment.includes('babelCallOSModule')||!fragment.includes('babelCallOSStyles')) throw new Error('Fragmento Call OS V2 inválido.');
...
const nextLoader=loader
  ...
  .replace(/\?v=\d+/g,'?v=8501');
fs.writeFileSync('index.html',nextLoader);

console.log(`Call OS V2 applied. HTML=${html.length} gzip-base64=${out.length} tail=${tail.length}`);
